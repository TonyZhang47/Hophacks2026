...
- Scan a bottle using a live camera preview, a chosen photo, or a phone linked by QR. Live camera needs a secure context (`localhost` or HTTPS). For a phone on a local HTTP network address, use Take photo or Choose a photo. Browser/device camera permission is required.
