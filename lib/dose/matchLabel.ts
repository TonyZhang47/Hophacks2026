...
function looksLikeDirections(q: string): boolean {
  return (
    /^(?:take|taking|takes|use|using|give|apply|by mouth)\b/i.test(q) ||
...
/**
 * Map OCR text to catalog medicines (RxNorm / bundled list). Grok extracts the printed
 * name against common_meds first. It may only pick an rxcui from that list. First catalog
 * search hit is the fallback if Grok is down. A deliberate Grok "none" stays null.
 */
